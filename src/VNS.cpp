...
VNSAlgorithm::VNSAlgorithm(Graph workingGraph) : 
  MaxMeanAlgorithm(workingGraph) {}

Solution VNSAlgorithm::resolve() {
  ...
  for (int currentSolution = 0; currentSolution < 1000; currentSolution++) {
    ...
    while (currentIteration < 3) {
      ...
      std::pair<double, std::vector<int>> solution4 = postProcess(solution3.first, solution3.second, false);
    ...
    if (_bestSolution.getSolutionScore() < (bestValue)) {
...
std::vector<std::pair<double, std::pair<int, int>>> VNSAlgorithm::preProcess() {
...
std::pair<double, std::vector<int>> VNSAlgorithm::construct(std::vector<int> solutionArray, double currentDistance, int kElementsOfLRC) {
...
std::pair<double, std::vector<int>> VNSAlgorithm::postProcess(double currentDistance, std::vector<int> solutionArray, bool greedyOrAnxious) {
...
std::pair<double, std::vector<int>> VNSAlgorithm::constructGreedy(double currentDistance, std::vector<int> solutionArray) {
...
std::pair<double, std::vector<int>> VNSAlgorithm::constructAnxious(double currentDistance, std::vector<int> solutionArray) {
  if (solutionArray.size() < getGraph().getNumberOfNodes()) {
    int numberOfElementsOnSolution = 0;
    // Mientras sigamos incluyendo elementos en las iteraciones seguimos ejecutando el código.
    do {
...
std::pair<double, std::vector<int>> VNSAlgorithm::shake(std::vector<int> solutionArray, double currentDistance, int neighborhoodDistance) {
